...
BEGIN
create table Tipo_Plato(codigoTipoPlato int auto_increment not null primary key, 
...
BEGIN
create table Servicio_Has_Empleado(servicios_codigoServicio int,
empleados_codigoEmpleado int,
fechaEvento date,
horaEvento time,
lugarEvento varchar(150),
foreign key(servicios_codigoServicio) references Servicio(codigoServicio) ON DELETE CASCADE,
foreign key(empleados_codigoEmpleado) references Empleado(codigoEmpleado) ON DELETE CASCADE);
END $$
DELIMITER ;
...
BEGIN
create table Producto_Has_Plato(producto_codigoProducto int,
platos_codigoPlato int,
foreign key(producto_codigoProducto) references Producto(codigoProducto),
foreign key(platos_codigoPlato) references Plato(codigoPlato) ON DELETE CASCADE);
END $$
DELIMITER ;
...
BEGIN
create table Servicio_Has_Plato(servicios_codigoServicio int,
platos_codigoPlato int,
foreign key(servicios_codigoServicio) references Servicio(codigoServicio),
foreign key(platos_codigoPlato) references Plato(codigoPlato) ON DELETE CASCADE);
END $$
DELIMITER ;
